...
/*Table structure for table `tipo_estado_movimiento` */

DROP TABLE IF EXISTS `tipo_estado_movimiento`;

CREATE TABLE `tipo_estado_movimiento` (
  `idEstado` bigint(20) NOT NULL,
  `Descripcion` text,
  PRIMARY KEY (`idEstado`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `tipo_estado_movimiento` */

insert  into `tipo_estado_movimiento`(`idEstado`,`Descripcion`) values 
(1,'Pendiente de envío al Banco Central.'),
(2,'Enviado al Banco Central o no era necesario enviarla.');
